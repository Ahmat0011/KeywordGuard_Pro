                ...
                if (config != null && config.IsActive())
                {
                    ...
                    foreach (var kw in config.Keywords)
                    {
                        ...
                        if (domain != null)
                        {
                            ...
                            // so that titles like "mail.ru - Google Chrome" are detected regardless of
                            // whether the keyword was added as aggressive or not.
                            // Only the full domain is used – no prefix extraction – to avoid false positives
                            // (e.g. "mail" from "mail.ru" must not block "Mail - Outlook").
